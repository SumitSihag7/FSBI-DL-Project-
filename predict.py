...
def predict_single(args, engine) -> dict:
    ...
    # ── Extract label & confidence from result dict ──
    label      = result["label"]
    ...
    if args.json:
        ...
        print(f"\n  {BOLD}{color}{label}{RESET}  {color}{bar}{RESET}  {confidence*100:.1f}%")
...
def _get_tensors(img_path: str, engine):
    """Get (rgb, fft) tensors for a single image — with batch dimension."""
    from PIL import Image
    from data.augmentations import preprocess_image, preprocess_fft
    img = Image.open(img_path).convert("RGB")
    rgb = preprocess_image(img, size=engine.image_size).unsqueeze(0)  # (1, 3, H, W)
    fft = preprocess_fft(img,   size=engine.image_size).unsqueeze(0)  # (1, 3, H, W)
    return rgb, fft
...
def _run_visualisation(args, engine, img_path: str, label: str, confidence: float):
    ...
    try:
        ...
        img = Image.open(img_path).convert("RGB")
        rgb = preprocess_image(img, size=engine.image_size).to(engine.device)  # (1,3,H,W)
        fft = preprocess_fft(img,   size=engine.image_size).to(engine.device)  # (1,3,H,W)
